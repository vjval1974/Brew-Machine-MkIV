  ...
  if (BrewState.ucStep < MAX_BREW_STEPS && Brew[BrewState.ucStep+1].ucWait == 1)
    {
      ...
      BrewState.ucStep++;
      sprintf(buf, "entering next step: %d, %s\r\n", BrewState.ucStep+1, Brew[BrewState.ucStep].pcStepName);
      vConsolePrint(buf);
      vBrewRunStep();
  }
  else
